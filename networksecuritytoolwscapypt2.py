...
host = ()
# IP to test 10.0.0.181, 10.0.0.80, 10.0.0.199
def tcpscan(host):
    host = input("\nEnter IP to perform tcp scan on: ")
    port_range = (20, 23, 80, 443, 3389)
...
def ping_sweep():
    ...
    sweepresp = sr1(IP(dst=str(host))/ICMP(),timeout=2,verbose=0,)
    
    if sweepresp is None:
        print(f"{host} is down or not responding")
    elif (
        int(sweepresp.getlayer(ICMP).type)==3 and
        int(sweepresp.getlayer(ICMP).code) in [1,2,3,9,10,13]
    ):
